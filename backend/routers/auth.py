...
@router.post("/register", response_model=TokenExchangeResponse, status_code=status.HTTP_201_CREATED)
async def register_local_user(payload: RegisterRequest, db: AsyncSession = Depends(get_db)):
    """Register a new local user using email and/or phone plus password."""
    ...
    if payload.phone:
        existing = await auth_service.get_user_by_phone(payload.phone)
        if existing:
...
@router.post("/login-email", response_model=TokenExchangeResponse)
...
@router.post("/login-phone", response_model=TokenExchangeResponse)
async def login_with_phone(payload: PhoneLoginRequest, db: AsyncSession = Depends(get_db)):
    """Login using phone + password."""
    auth_service = AuthService(db)
    user = await auth_service.get_user_by_phone(payload.phone)
    if not user or not user.password_hash:
