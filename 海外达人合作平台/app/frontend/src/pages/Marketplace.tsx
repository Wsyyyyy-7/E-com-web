        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-4 relative z-10">
          ...
          <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto mb-8">
            <div className="text-center bg-white/5 backdrop-blur-sm rounded-xl p-3 border border-white/10">
              ...
              <p className="text-xs text-gray-400">活跃项目 Active</p>
            </div>
            <div className="text-center bg-white/5 backdrop-blur-sm rounded-xl p-3 border border-white/10">
              <div className="flex items-center justify-center gap-1.5 mb-1">
                <DollarSign className="w-4 h-4 text-emerald-400" />
                <span className="text-2xl font-bold text-white">¥{(totalBudget / 1000).toFixed(0)}k</span>
              </div>
              <p className="text-xs text-gray-400">总预算 Budget</p>
            </div>
            <div className="text-center bg-white/5 backdrop-blur-sm rounded-xl p-3 border border-white/10">
              <div className="flex items-center justify-center gap-1.5 mb-1">
              ...
              <p className="text-xs text-gray-400">申请人数 Applicants</p>
            </div>
          </div>
            ...
            <div className="bg-gray-900/80 backdrop-blur-sm rounded-xl border border-white/10 p-2 flex gap-2">
              <div className="flex-1 relative">
                ...
                <Input
                  ...
                  placeholder="搜索合作项目 Search campaigns..."
                  value={searchQuery}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
            ...
            <p className="text-lg font-medium text-gray-400">没有找到匹配的项目</p>
            <p className="text-sm mt-1 text-gray-500">试试调整筛选条件</p>
          </div>
        ) : (
            ...
            {filtered.map(c => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <Badge className="bg-white/5 text-gray-300 hover:bg-white/5 border border-white/10 text-xs gap-1">
                    {platformIcons[c.platform] || '📱'} {c.platform}
                  </Badge>
                  <Badge className="bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/10 border-emerald-500/20 text-xs">
                    {c.status === 'active' ? '招募中' : c.status}
                  </Badge>
                </div>
                ...
                <div className="border-t border-gray-800 pt-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {c.deadline_apply ? new Date(c.deadline_apply).toLocaleDateString('zh-CN') : '长期'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-12 grid sm:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-white group-hover:text-sky-400 transition-colors">
                浏览达人目录 Browse Creators
              </h3>
              <p className="text-sm text-gray-500 mt-0.5">查看海外达人的 profile、履约率和报价</p>
            </div>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500 mt-0.5">了解商家的过往合作历史和信誉</p>
            </div>
      ...
      <Dialog open={!!selectedCampaign} onOpenChange={() => setSelectedCampaign(null)}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-gray-900 border-gray-800 text-white">
          {selectedCampaign && (
            <>
              <DialogHeader>
                <DialogTitle className="text-xl text-white">{selectedCampaign.title}</DialogTitle>
              </DialogHeader>
              <div className="space-y-5 mt-4">
                <p className="text-gray-400">{selectedCampaign.description}</p>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: '国家 Country', value: `${COUNTRIES.find(co => co.code === selectedCampaign.country)?.flag || ''} ${selectedCampaign.country}` },
                    { label: '平台 Platform', value: `${platformIcons[selectedCampaign.platform] || ''} ${selectedCampaign.platform}` },
                    { label: '类目 Category', value: selectedCampaign.category },
                    { label: '总预算 Budget', value: `¥${selectedCampaign.total_budget?.toLocaleString()}` },
                    { label: '单人报酬 Per Creator', value: `¥${selectedCampaign.per_creator_min} - ¥${selectedCampaign.per_creator_max}` },
                    { label: '保留期 Retention', value: `${selectedCampaign.retention_days} 天` },
                  ].map((item, i) => (
                ...
                <div>
                  <h4 className="font-medium text-white mb-2">验收条件 Requirements</h4>
                ...
                <div>
                  <h4 className="font-medium text-white mb-2">里程碑 Milestones</h4>
                  <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  <Button
                    ...
                    onClick={() => { setSelectedCampaign(null); navigate('/creator'); }}
                  >
                    申请合作 Apply Now <ArrowRight className="w-4 h-4 ml-1" />
                  </Button>
